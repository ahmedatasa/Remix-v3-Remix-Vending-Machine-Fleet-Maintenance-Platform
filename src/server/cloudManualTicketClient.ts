...
export async function syncManualTicketToCloud(
  ...
  const clientId =
    (process.env.SYNC_CLIENT_ID || 'ksu-desktop-sync-client-2026').trim();
  const clientSecret = (process.env.SYNC_CLIENT_SECRET || '').trim();

  if (!base || !clientId || !clientSecret) {
    return { status: 'FAILED', reason: 'CLOUD_SYNC_NOT_CONFIGURED' };
  }

  const actor = req?.user || {};
  const actorId = String(actor.id || '').trim();
  const actorName = String(
    actor.fullName || actor.name || actor.username || actorId
  ).trim();
  const actorRole = String(actor.role || '').trim().toUpperCase();

  if (!actorId || !actorName || !actorRole) {
    return { status: 'FAILED', reason: 'MANUAL_TICKET_ACTOR_REQUIRED' };
  }

  const machineId = String(machine?.id || '').trim();
  const publicQrToken = String(machine?.publicQrToken || '').trim();

  if (!machineId || !publicQrToken) {
    return { status: 'FAILED', reason: 'MACHINE_CLOUD_IDENTITY_MISSING' };
  }
  ...
  try {
    const response = await fetch(`${base}/sync/manual-ticket`, {
      method: 'POST',
      redirect: 'error',
      ...
      body: JSON.stringify({
        ...
        mainTicketNumber: ticket.ticketNumber,
        integrationMachineId: machineId,
        publicQrToken,
        category: ticket.category || 'OTHER',
        description: ticket.description || 'Manual ticket entry',
        reporterName:
          ticket.reporterName ||
          actor.fullName ||
          actor.name ||
    ...
    if (!response.ok) {
      ...
      return {
        status: 'FAILED',
        reason: safeCodes.has(body?.error)
          ? body.error
    ...
    return {
      status: 'SYNCED',
      cloudTicketId: body.ticketId,
      cloudReportId: body.cloudReportId,
      publicTrackingToken: body.trackingToken
    };
  } catch {
